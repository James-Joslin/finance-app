...
import { CalendarClock, Check, ChevronDown, Download, FileUp, Filter, History, RefreshCw, RotateCcw, Search, WalletCards } from 'lucide-react';
...
import { mutations, queryKeys, useAccounts, useCategories, useFinovaMutation, useImportRows, useImports, useTransactions } from '../lib/queries';
...
const importInvalidations = [
    ...
    queryKeys.budgets, queryKeys.safety, queryKeys.occurrences,
    ['transaction-imports'], ['transaction-import-rows'],
];
...
function ImportModal({ open, onClose, accounts }) {
    ...
    const commit = useFinovaMutation(mutations.commitImport, importInvalidations);
    const undo = useFinovaMutation(mutations.undoImport, importInvalidations);
    const batch = commit.data || preview.data;
    const rows = useImportRows(batch?.id, { outcome: rowOutcome || undefined, page: rowPage, pageSize: 50 },
        open && tab === 'new' && Boolean(batch));
    const history = useImports(
        { accountId: historyAccountId || undefined, page: historyPage, pageSize: 10 },
        open && tab === 'history' && Boolean(historyAccountId),
    );
    const historyRows = useImportRows(expandedId, { page: historyRowPage, pageSize: 50 },
        ...
        <Modal open={open} onClose={close} wide title="Import transactions" copy="Preview every row before importing, review previous batches, or undo the latest import for an account.">
            ...
            {tab === 'new' ? (
                    ...
                    {!batch ? (
                        <form className="form-stack" onSubmit={submitPreview}>
                            ...
                            <label className="file-drop">
                                ...
                                <small>Maximum 10 MB · the original file is not retained</small>
                                <input type="file" accept=".ofx,.qif,.pdf,application/pdf" onChange={(event) => setFile(event.target.files[0] || null)} />
                            </label>
                            {preview.error && <p className="form-error">{apiError(preview.error)}</p>}
                            <div className="modal-actions"><button type="button" className="button secondary" onClick={close}>Close</button><button className="button" disabled={!accountId || !file || preview.isPending}>{preview.isPending ? 'Preparing preview…' : 'Preview transactions'}</button></div>
                        </form>
                    ) : (
                        <>
                            <ImportSummary batch={batch} />
                            <div className="import-row-toolbar">
                                <strong>{batch.status === 'preview' ? 'Preview rows' : 'Final results'}</strong>
                                <select aria-label="Filter import rows" value={rowOutcome} onChange={(event) => { setRowOutcome(event.target.value); setRowPage(1); }}>
                                    <option value="">All outcomes</option>
                                    {batch.status === 'preview' && <option value="ready">Ready</option>}
                                    {batch.status !== 'preview' && <option value="imported">Imported</option>}
                                    <option value="skipped">Skipped</option>
                                    <option value="rejected">Rejected</option>
                                </select>
                            </div>
                            <ImportRows query={rows} page={rowPage} onPage={setRowPage} />
                            {(commit.error || rows.error) && <p className="form-error">{apiError(commit.error || rows.error)}</p>}
                            <div className="modal-actions">
                                {batch.status === 'completed'
                                    ? <><button type="button" className="button secondary" onClick={close}>Close</button><button type="button" className="button" onClick={startAgain}>Import another</button></>
                                    : <><button type="button" className="button secondary" onClick={startAgain}>Cancel preview</button><button type="button" className="button" disabled={batch.importable === 0 || commit.isPending} onClick={() => commit.mutate(batch.id)}>{commit.isPending ? 'Importing…' : `Import ${batch.importable} transactions`}</button></>}
                            </div>
                        </>
                    )}
                </div>
                ...
                <ImportHistory
                    ...
                    onAccount={(value) => { setHistoryAccountId(value); setHistoryPage(1); setExpandedId(null); }}
                    query={history}
                    page={historyPage}
                    onPage={setHistoryPage}
                    expandedId={expandedId}
                    onExpand={(id) => { setExpandedId(expandedId === id ? null : id); setHistoryRowPage(1); }}
                    rows={historyRows}
...
export function ImportSummary({ batch }) {
    return (
        <section className="import-summary">
            <div><span><small>{batch.accountName}</small><strong>{batch.fileName}</strong></span><Pill tone={batch.status === 'completed' ? 'success' : batch.status === 'undone' ? 'neutral' : 'info'}>{batch.status}</Pill></div>
            <div className="import-totals">
                <span><strong>{batch.status === 'preview' ? batch.importable : batch.imported}</strong><small>{batch.status === 'preview' ? 'Ready' : 'Imported'}</small></span>
                <span><strong>{batch.skipped}</strong><small>Duplicates</small></span>
                <span><strong>{batch.rejected}</strong><small>Rejected</small></span>
                <span><strong>{batch.total}</strong><small>Total rows</small></span>
            </div>
        </section>
    );
}
...
export function ImportRows({ query, page, onPage }) {
    if (query.isLoading) return <div className="import-rows-state"><RefreshCw className="spin" /> Loading row results…</div>;
    if (query.error) return null;
    if (!query.data?.items?.length) return <div className="import-rows-state">No rows match this outcome.</div>;
    return (
        <div className="import-rows-wrap">
            ...
            <ImportPagination page={page} totalPages={query.data.totalPages} totalItems={query.data.totalItems} onChange={onPage} label="rows" />
        </div>
    );
}

function ImportHistory({ accounts, accountId, onAccount, query, page, onPage, expandedId, onExpand, rows, rowPage, onRowPage, onUndo, undo, onClose }) {
    return (
        <div className="form-stack">
            <Field label="Account" hint="Undo is available only on the latest active import for this account."><select value={accountId} onChange={(event) => onAccount(event.target.value)}><option value="">Choose an account</option>{accounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></Field>
            {!accountId ? <div className="import-rows-state">Choose an account to view its import history.</div>
                : query.isLoading ? <div className="import-rows-state"><RefreshCw className="spin" /> Loading import history…</div>
                : query.error ? <p className="form-error">{apiError(query.error)}</p>
                : query.data?.items?.length === 0 ? <div className="import-rows-state">No completed imports for this account.</div>
                : <div className="import-history-list">
                    {query.data?.items.map((item) => (
                        <article className="import-history-item" key={item.id}>
                            <button type="button" className="import-history-trigger" onClick={() => onExpand(item.id)} aria-expanded={expandedId === item.id}>
                                <span><History /><span><strong>{item.fileName}</strong><small>{relativeDate(item.completedAt || item.createdAt)} · {item.imported} imported · {item.skipped} skipped · {item.rejected} rejected</small></span></span>
                                <span><Pill tone={item.status === 'completed' ? 'success' : 'neutral'}>{item.status}</Pill><ChevronDown /></span>
                            </button>
                            {expandedId === item.id && <div className="import-history-detail"><ImportRows query={rows} page={rowPage} onPage={onRowPage} />{item.canUndo && <button type="button" className="button danger import-undo" disabled={undo.isPending} onClick={() => onUndo(item)}><RotateCcw />{undo.isPending ? 'Undoing…' : 'Undo this import'}</button>}</div>}
                        </article>
                    ))}
                    <ImportPagination page={page} totalPages={query.data?.totalPages || 1} totalItems={query.data?.totalItems || 0} onChange={onPage} label="imports" />
                </div>}
            {undo.error && <p className="form-error">{apiError(undo.error)}</p>}
            <div className="modal-actions"><button type="button" className="button secondary" onClick={onClose}>Close</button></div>
        </div>
    );
}
...
function ImportPagination({ page, totalPages, totalItems, onChange, label }) {
    ...
    return (
        <div className="import-pagination">
            ...
            <div><button type="button" className="icon-button" disabled={page <= 1} onClick={() => onChange(page - 1)}>‹</button><strong>{page} / {totalPages}</strong><button type="button" className="icon-button" disabled={page >= totalPages} onClick={() => onChange(page + 1)}>›</button></div>
        </div>
    );
}
